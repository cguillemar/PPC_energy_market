...
class Marche(Process):
    def __init__(self, vPrix, tabMarche, listTemp, portefeuille, flagFinishTurn, monJour, synCondition,suiviMaison,tabProduction,nMaison):
        super().__init__()
        self.vPrix = vPrix
    ...
    def run(self):
        ...
        while self.monJour.value != 365:
            ...
            fichier.write("Jour"+str(self.monJour.value)+"\n")
            fichier.close()
    ...
    def __achat_marche__(self, i, valeur):
        ...
        fichier= open("nrj.txt","a")
        fichier.write("Maison "+str(i)+" échange avec le marché "+str(valeur)+"\n")
        fichier.close()
...
class Maison(Process):
    ...
    def __donnerMaison__(self):
        ...
        fichier= open("nrj.txt","a")
        fichier.write("Maison "+str(numMaison)+" donne aux maisons "+str(self.production-self.consommation)+"\n")
        fichier.close()
    ...
    def __acheterCommuniste__(self):
        ...
        if self.tabProduction.empty() == False:
            with self.lockProduction:
                ...
                if cagnotteProdMaisons >= (self.consommation-self.production):
                    ...
                    fichier= open("nrj.txt","a")
                    fichier.write("Maison "+str(numMaison)+" achète aux maisons "+str(self.consommation-self.production)+"\n")
                    fichier.close()
        ...
        if self.production<self.consommation:
            ...
            fichier= open("nrj.txt","a")
            fichier.write("Maison "+str(numMaison)+" achète au marché "+str(self.consommation-self.production)+"\n")
            fichier.close()
...
if __name__ == "__main__":
    ...
    # On lance le marché
    pMarche = Marche(vPrix, tabMarche, listTemp, portefeuille, flagFinishTurn, monJour, synCondition,suiviMaison,tabProduction,nMaison)
    pMarche.start()
    ...
    b=time.time()
    fich = open("temps.txt","a")
    fich.write(str(b-a)+","+str(nMaison)+"\n")
    fich.close()
