...
class Marche(Process):
    def __init__(self, vPrix, tabMarche, listTemp, portefeuille, flagFinishTurn, monJour, synCondition,suiviMaison,tabProduction):
        super().__init__()
        self.vPrix = vPrix
        ...
        while self.monJour.value != 365:
            ...
            while Marche.__test__(self) != True:
                ...
                if self.tabProduction.empty() == False:
                    indice, valeur = self.tabProduction.get()
                    print("valeur",valeur)
                    nrj=nrj-valeur
                    #print(indice,valeur)
                    threadVenteMarche = threading.Thread(target=Marche.__vente_marche__, args=(self, indice, valeur))
                    threadVenteMarche.start()
                    threadVenteMarche.join()
            ...
            self.suiviMaison[self.monJour.value] = self.portefeuille[3]
    ...
    def __achat_marche__(self, i, valeur):
        ...
        self.portefeuille[i] = ancienneVal - abs(valeur) * ((self.vPrix[self.monJour.value])+1.5)
    ...
    def __vente_marche__(self, i, valeur):
        ancienneVal=self.portefeuille[i]
        self.portefeuille[i] = ancienneVal + abs(valeur) * ((self.vPrix[self.monJour.value])+1.5)
...
class Maison(Process):
    ...
    def __donnerMaison__(self,lockProduction):
        ...
        self.tabProduction.put(tupleSend)
        self.consommation = self.production
...
if __name__ == "__main__":
    ...
    # On lance le marché
    pMarche = Marche(vPrix, tabMarche, listTemp, portefeuille, flagFinishTurn, monJour, synCondition,suiviMaison,tabProduction)
    pMarche.start()
    ...
    if monJour.value == 365:
        ...
        fig =plt.figure()
        plt.subplot(2,1,1)
        plt.plot(M, P,label="Mode Capitaliste")
