...
def meteo(listTemp, N):
    ...
    temp = [3.3, 4.2, 7.8, 10.8, 14.3, 17.5, 19.4, 19.1, 16.4, 11.6, 7.2,4.2,3.1,4.5]  # Temperatures moyennes sur 1 année de janvier à décembre
...
class Marche(Process):
    def __init__(self, vPrix, tabMarche, listTemp, portefeuille, flagFinishTurn, monJour, synCondition,suiviMaison,tabProduction,nMaison,suiviMaison2):
        super().__init__()
        self.vPrix = vPrix
...
class Maison(Process):
    ...
    def __donnerMaison__(self):
        ...
        with self.lockProduction:
            if self.tabProduction.empty()==False:
                ...
                fichier = open("nrj.txt","a")
                fichier.write("Maison "+str(numMaison)+" donne "+str(self.production-self.consommation)+" kW"+"\n")
                fichier.close()
    ...
    def __acheterCommuniste__(self):
        ...
        if self.tabProduction.empty() == False:
            with self.lockProduction:
                ...
                if cagnotteProdMaisons >= (self.consommation-self.production):
                    ...
                    fichier = open("nrj.txt","a")
                    fichier.write("Maison "+str(numMaison)+" achète "+str(self.production-self.consommation)+" kW"+"\n")
                    fichier.close()
                    ...
                    fichier = open("nrj.txt","a")
                    fichier.write("Maison "+str(numMaison)+" achète "+str(cagnotteProdMaisons)+" kW"+"\n")
                    fichier.close()
        ...
        if self.production<self.consommation:
            ...
            fichier = open("nrj.txt","a")
            fichier.write("Maison "+str(numMaison)+" achète au marché "+str(self.production-self.consommation)+" kW"+"\n")
            fichier.close()
    ...
    def __echangeMarche__(self):
        ...
        fichier = open("nrj.txt","a")
        fichier.write("Maison "+str(numMaison)+" échange au marché "+str(self.production-self.consommation)+" kW"+"\n")
        fichier.close()
...
if __name__ == "__main__":
    ...
    etatSimulation=int(input("Quel mode de simulation voulez vous faire ? (0 : communiste, 1: capitaliste) "))  #0 vaut communiste 1 vaut capitaliste
    ...
    flagFinishTurn = Array('i', [False]*nMaison)  # se met a True chaque jour quand maison a finie ses echanges pour prevenir marche
    ...
    # On lance le marché
    pMarche = Marche(vPrix, tabMarche, listTemp, portefeuille, flagFinishTurn, monJour, synCondition,suiviMaison,tabProduction,nMaison,suiviMaison2)
    pMarche.start()
